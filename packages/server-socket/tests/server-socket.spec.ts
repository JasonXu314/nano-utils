...
describe('Socket test suite', () => {
	...
	it('Should throw an error if attempting to send through a closed socket', async () => {
		return new Promise<void>((resolve) => {
			wss = new WebSocket.Server({ port: 5000 });
			socket = new WebSocket('ws://localhost:5000');
			...
			wss.on('connection', (sock) => {
				...
				socket.close().then(() => {
					...
					}).toThrowError('Socket#close(): Attempted to send message on closed socket.');
	...
	it('Should handle binary data correctly', async () => {
		return new Promise<void>((resolve) => {
			...
			socket.on('message', (data) => {
				...
				expect(msg).toMatchObject({ type: 'TEST_2', test2: 'hi' });
				socket.send(Buffer.from(JSON.stringify({ type: 'TEST_2', test2: 'hi' })));
			});

			socket.on('open', () => {
				socket.send(Buffer.from(JSON.stringify({ type: 'TEST', test: 'hi' })));
			});
		});
	});
